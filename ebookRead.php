 ...
 ***********/
	...
	/**
	 * Tests if a optional attribute exists and then return it. If an array of tags is sent in then it returns
	 ...
	 * only that else we send back the whole array.
	 **/
	...
	private function xmlFindData(SimpleXMLElement $xmlInput, $tag){
		...
		foreach($xmlInput->children() as $child){
			...
			if(isset($recurse)){
				if(is_array($recurse)){
					foreach($recurse as $element){
						array_push($array, $element);
					}
				}else{
					array_push($array, $recurse);
				}
			}
		}
		return $array;
 ...
 *********************/
	...
	public function getContentFile($location){
		if(isset($this->ebookData->contentFolder)){
			...
			return $this->readEpubFile($this->ebookData->epub, $location);
		}else{
